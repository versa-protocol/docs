...
**Primary event types** — all use the [Versa schema](/schema):
...
- **`itinerary`** - Travel information without financial data. See the [Itinerary Schema](/itinerary).
...
When sending travel confirmations without payment details attached, you can register an `itinerary` event. See the [Itinerary Schema](/itinerary) for the reduced field set.
