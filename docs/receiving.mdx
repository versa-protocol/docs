...
1. Via a **[custodial configuration](#custodial-configuration)**. With this arrangement, you'll receive webhook events containing the receipt JSON at the URL you specify. Versa performs the decryption.
...
> [!NOTE]
> Versa also supports `card_bin` and `card_last_four` handles, but to enable those forms of matching, you will need to be opted in by the Versa support team.
...
If you've set up a custodial client, you will receive decrypted receipt objects at your webhook address. You're free to store and [render](#rendering-receipts) these receipts within your product.
...
Receipts are decrypted with the AES-GCM-SIV 256 bit encryption algorithm, using the registry-provided key and a unique nonce provided by the sender.
