...
You are responsible for collecting the necessary permissions when registering customers. For `customer_email` registrations, the email addresses you add will also be subject to a verification step. When you register an email address, the registration will be put into a pending state. Versa sends a verification email to the address; you will start to receive matching receipts only after the verification link contained in that email is clicked.
...
Once you've set up your client, you can test your integration by sending a test receipt from the Versa web app. Look for the 'Test Receipt' button on your [Client](https://app.versa.org/client) page.This will send a simple receipt to your webhook address, allowing you to verify that you're correctly receiving and processing receipts.
