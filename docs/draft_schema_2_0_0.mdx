---
title: "Draft Schema 2.0.0"
---

# Draft Schema 2.0.0

This is a draft of the next major version of the Versa receipt schema, version 2.0.0.

## Key Changes

This is a major version update to the schema,
changing the `passenger` string to a nullable `Person` object,
along with other small improvements.

### Breaking Changes

- `passenger` changes from type `["string", "null"]` to a nullable `Person` object
- `metadata` is removed from flight tickets

### Other Changes

- Fixed: `place` nullable fields should also not be required
- Fixed: `vehicle` nullable `image` should also not be required
- Fixed: `currency` should support `cny` instead of `cnh`
- Added to rental car `vehicle`
  - `license_plate_number` (string)
  - `vehicle_class` (ACRISS car code, four letters)
- Added `adjustment_type` enum option: `add_on`
- Added `website` to `customer`
- In a lodging itemization, `guests` is an array of `person` objects

## New Passenger Model

A `Person` is distinct from a `Customer` in that it always refers to an individual.
...
| `guests`                    | `array`           | Array of [person](#person) objects.                                                                                                                             |
...
For SaaS bills, phone bills, etc. View a complete [subscription receipt](https://versa.org/studio?sender=generic&receipt=subscription) example.
...
| `current_period_start_at` | `nullable integer` | Start of the current period of the subscription. Measured in seconds since the Unix epoch.                                                                          |
| `current_period_end_at`   | `nullable integer` | End of the current period.                                                                                                                                          |
