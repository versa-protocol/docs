...
- **`receipt`**: A completed transaction. Includes [`payments`](#payments).
- **`booking`**: A travel reservation. Payments are not required, and `header.paid` is typically zero. Bookings are only valid for travel itemization types: `car_rental`, `flight`, `lodging`, or `transit_route`. See the [Booking Schema](/booking) for details.
...
- **`itinerary`**: Travel information without financial data. Excludes totals, taxes, and payments. Focused on travel details only. See the [Itinerary Schema](/itinerary) for details.

## Sections

Schema objects are composed of four sections:

1. [**Header**](#header): Top-level fields, e.g. merchant details, totals, transaction date.
2. [**Itemization**](#itemization): Description, quantity, unit cost, and total for each line item. Type-specific templates describe different purchase types, e.g. flight, e-commerce purchase, service.
...
4. [**Payments**](#payments): Payment status and details. Required for `receipt` and `invoice` event types; omitted for `itinerary`; optional for `booking`.
...
| `total`            | `integer`         | Total amount. In the currency specified and in the [smallest currency unit](#currency-values).              |
...
| `booked_at`        | `nullable integer`| Time at which the transaction was booked. Measured in seconds since the Unix epoch.                         |
| `trip`             | `nullable object` | Optional [trip](#trip) grouping — for associating the transaction with a broader trip.                      |
| `lifecycle_status` | `nullable enum`   | One of `active`, `canceled`, or `refunded`. Used to communicate the current state of the transaction.       |
...
| `booker`   | `nullable object` | A [person](#person) object for the person who arranged the purchase (booked, bought). |
...
Optional grouping used to associate a transaction with a broader trip (e.g. a multi-leg itinerary composed of flight, lodging, and car rental transactions). All fields are optional — include whichever are useful for your use case.
