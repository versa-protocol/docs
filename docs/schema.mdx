...
All primary event types share the same underlying schema structure, but differ in which fields are expected to be present. Specify the event type via the `event_type` parameter when sending via the custodial API or registering via the registry.

| Event Type | Description |
| ---------- | ----------- |
| `receipt` | A completed transaction. Includes [`payments`](#payments) and `header.paid` is greater than zero. Receipts are the most common event type. |
| `booking` | A pre-purchase reservation. Payments are not required, and `header.paid` is typically zero. Bookings are only valid for travel itemization types: `car_rental`, `flight`, `lodging`, or `transit_route`. |
...
| `itinerary` | Travel information without financial data. Excludes totals, taxes, and payments. Focused on travel details only — no e-commerce itemizations. See the [Itinerary Schema](/itinerary) for details. |
...
> [!NOTE]
> For `booking` event types, only the travel itemization types are valid: `car_rental`, `flight`, `lodging`, or `transit_route`. For `itinerary` event types, `general` and `ecommerce` are also excluded. See the [Itinerary Schema](/itinerary) for the full itinerary itemization spec.
