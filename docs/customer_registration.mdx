...
Senders, on the other hand, can only register customers to receive _their_ receipts. E.g. a travel manager admin could opt to push their American Airlines receipts to their expense management app on the American Airlines website (via their AA company settings screen), but they would not be able to push their Delta Airlines receipts to their expense management app via the American Airlines website.

Technically speaking, these receiver vs. sender registration scopes are configured via the `receiver_client_id` parameter on the /handle endpoint. Receiver clients do not need to specificy a `receiver_client_id` — they can either skip the paramter, or set the value to their own `client_id` (anything else will error). Sender clients attempting to register customers, on the other hand, must include a `receiver_client_id`, and that `receiver_client_id` must reference an active Receiver Client. If they exclude this parameter, the registration attempt will error.
