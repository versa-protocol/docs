...
Then, as you make sales, you'll check if those customers are registered to receive your receipts. When Versa returns a positive customer match, you'll send them the itemized receipt object.

The receipt object itself must conform to Versa's [Receipt Schema](/receipt-schema). The schema offers a range of template types and options to match the goods and services you sell; you'll map from your internal purchase data accordingly. Configure your merchant name, logo, and brand color on the [Profile](https://app.versa.org/profile) settings screen, and then prototype and preview your full, branded receipts in the [Studio](https://app.versa.org/studio).
