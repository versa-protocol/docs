...
1. Via a **[custodial configuration](#custodial-configuration)**: With this arrangement, sending a receipt is as simple as a standard HTTP `POST` request, via a RESTful API. You make a single call per receipt, passing up the receipt JSON to Versa.
...
[Create your client](https://app.versa.org/client) in the Versa web app, and select that you are a 'Sender' and that you'd like to enable the 'Custodial' mode. Then copy your Client Secret and save it someplace safe.

You're now ready to send your first receipt. Add your `CLIENT_ID`, `CLIENT_SECRET`, a customer 'handle' (e.g. their email address), and a Versa receipt object.
...
Versa routes receipts according to the customer handles you provide. Customers must explicitly authorize 'receivers' to receive their receipts (see the [Receiving](/receiving) docs for details). When you complete a sale, you check if there are any matching authorized receivers. The receipt is only sent in the case that there is a positive match.
...
Versa supports four different handle types: `customer_email`, `customer_email_domain`, `card_bin`, `card_last_four`. You must send at least one handle, and you can include multiple. Versa will return matches for any of the handles independently, with the exception of `card_last_four` (`card_last_four` is never unique).
