...
Versa supports four different handle types: `customer_email`, `customer_email_domain`, `card_bin`, `card_last_four`. You must sand at least one handle, and up to the full set of four. Versa will return matches for any of the handles, with the exception of `card_last_four` (as `card_last_four` is never unique).
