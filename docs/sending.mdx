...
Versa is able to route receipts based on the customer handle you provide. Customers must explicitly authorize 'receivers' to receive their receipts (see the [Receiving](/receiving) docs). When you complete a sale, you check if there are any matching authorized receivers. Only in the case that there is a positive match is the receipt sent.
