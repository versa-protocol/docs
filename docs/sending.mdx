...
## (Optional) Checking registration

For application purposes, it may be useful to check the registration of a hypothetical transaction before registering it.
This can be done by posting a registration request to the `regcheck` endpoint instead of to `register`.
The `regcheck` endpoint expects the same body JSON as the `register` endpoint, but returns a list of matched receiver profiles instead of their addresses and secrets.
...
```json
{
  ...
  "receivers": [
    {
      "client_id": "versa_cid_prod_05d108be2b974577b203a4d907a61e7d,
      "receiver": {
