...
Versa supports four different handle types: `customer_email`, `customer_email_domain`, `card_bin`, `card_last_four`.

```bash
curl --request POST \
  --url 'https://registry.versa.org/http/register' \
  --header 'Authorization: Basic <CLIENT_ID:CLIENT_SECRET>' \
  --header 'Content-Type: application/json' \
  --data '{
...
| `card_bin`              | `nullable string` | The first 4-8 digits of a card number, identifying the financial institution that issued the card. E.g. 412345. |
| `card_last_four`        | `nullable string` | The last four digits of a card number.                                                                          |
