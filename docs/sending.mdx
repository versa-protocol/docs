...
As you make sales, you will check if each sale is to a customer who has registered to receive receipts. If Versa returns a customer match, you will adapt your internal purchase data to Versa's [Receipt Schema](/receipt-schema), and pass that receipt object along to your customer.

Configure your merchant name, logo, and brand color on the [Profile](https://app.versa.org/profile) settings screen, and then prototype and preview your full, branded receipts in the [Studio](https://app.versa.org/studio).
...
You're now ready to send your first receipt. Add your Client ID, Client Secret, the email domain of your customer, and a Versa receipt object. For test sending, you can use 'sandbox-firehose.versa.org' as the `customer_email_domain` and you can copy one of the sample receipts from the [receipt schema documentation](/receipt-schema). Read the [testing section](#testing-your-connection) for more details.
