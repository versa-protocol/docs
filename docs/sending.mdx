...
You're now ready to send your first receipt. Add your Client ID, Client Secret, the 'handle' of your customer (typically email), and a Versa receipt object.
...
Versa supports four different handle types: `customer_email`, `customer_email_domain`, `card_bin`, `card_last_four`. You must send at least one handle, and up to the full set of four. Versa will return matches for any of the handles independently, with the exception of `card_last_four` (as `card_last_four` is never unique).
...
For a language-specific detailed implementation guide, see the [Sending Client Implementation](/sending_client) doc.
