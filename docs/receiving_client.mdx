...
Developer documentation for implementing a self-hosted Versa receiver client, using the [Versa REST API](/receiver). We currently maintain example client implementations for [Rust](https://github.com/versa-protocol/rust-client) and [NodeJS](https://github.com/versa-protocol/nodejs-client). Contact us if you'd like assistance with another language.
