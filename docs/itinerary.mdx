...
The itinerary is one of the four primary [Versa event types](/schema#event-types). It shares the same underlying schema as a receipt, booking, or invoice, but excludes all financial data (totals, taxes, payments, etc.) and is focused only on travel information. E-commerce itemizations are not supported.

A JSON Schema document and corresponding TypeScript Types are available [on GitHub](https://github.com/versa-protocol/schema/blob/main/data/itinerary.schema.json) or via the [@versa/schema](https://www.npmjs.com/package/@versa/schema) package.

## Wrapper

Itineraries use the same top-level wrapper as the [Versa schema](/schema#wrapper), but without the `payments` field.
...
The header mirrors the [schema header](/schema#header), but with all financial info (totals, etc.) removed.
...
| `booked_at`        | `nullable integer` | Time at which the itinerary was booked. Measured in seconds since the Unix epoch.                     |
| `trip`             | `nullable object`  | Optional [trip](/schema#trip) grouping — for associating the itinerary with a broader trip.           |
| `lifecycle_status` | `nullable enum`    | One of `active`, `canceled`, or `refunded`. Used to communicate the current state of the itinerary.   |

## Itemization

The Itemization section describes the record of services that compose the itinerary. Based on the type of purchase, the object conforms to one of five templates: [Car Rental](#car-rental), [Flight](#flight), [Lodging](#lodging), [Service](#service), or [Transit Route](#transit-route). Exactly one itemization template must be non-null.
...
| `rental_location`      | `object`           | A [place](/schema#place) describing the rental location.                |
| `return_location`      | `object`           | A [place](/schema#place) describing the return location.                |
...
| `metadata`             | `array`            | Array of [metadata](/schema#metadata) objects, for any additional info. |
...
| `location`            | `nullable object` | A [place](/schema#place) object describing the hotel / lodging.                                               |
...
For airport parking, SaaS subscriptions, phone bills, etc. View a complete [service itinerary](https://raw.githubusercontent.com/versa-protocol/schema/refs/heads/main/examples/itinerary/service.json) example.
...
| Field                     | Type               | Description                                                                                                                   |
| ------------------------- | ------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| `service_location`        | `nullable object`  | A [place](/schema#place) object describing the location where the service was provided (e.g., parking lot, service center). |
...
| `metadata`                | `array`            | Array of key/value [metadata pairs](/schema#metadata), e.g. SKU.                                                            |
...
| Field                | Type               | Description                                                                                                       |
| -------------------- | ------------------ | ----------------------------------------------------------------------------------------------------------------- |
| `departure_location` | `object`           | The origin [place](/schema#place) object.                                                                         |
| `arrival_location`   | `object`           | The destination [place](/schema#place) object.                                                                    |
| `departure_at`       | `nullable integer` | Departure time. Measured in seconds since the Unix epoch.                                                         |
| `arrival_at`         | `nullable integer` | Arrival time. Measured in seconds since the Unix epoch.                                                           |
...
| `metadata`           | `array`            | Array of key/value [metadata pairs](/schema#metadata).                                                            |
