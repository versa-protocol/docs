...
https://github.com/versa-protocol/schema/blob/tcm/itinerary/data/itinerary.schema.json
...
| `footer`         | `object` | The [footer](/receipt#footer) includes actions and supplemental text. |
...
| `location`    | `nullable object` | [Place](/receipt#place) object for brick-and-mortar in-person purchases.           |

## Itemization

The Itemization section describes the goods and services that compose the purchase. Based on the type of purchase, the object conforms to one of seven templates: [General](#general), [Car Rental](#car-rental), [Flight](#flight), [Lodging](#lodging), or [Transit Route](#transit-route). Exactly one itemization template must be non-null. When in doubt, use the 'general' template.
