...
<TabGroup>
  <Tab title="Rust">
    <CodeWrap title="protocol/lib.rs" fileUrl="https://github.com/versa-protocol/rust-client/blob/main/api_sender/src/routes/mod.rs">

    ```rust
    let schema_version = "1.5.1";

    // Initialize a Sending Client from the versa crate
    let versa_client = versa::client::VersaClient::new(registry_url, client_id, client_secret).sending_client(schema_version.into()).unwrap()

    // 1. Register the receipt with Versa registry
    let registration_response = versa_client.register_receipt(handles, transaction_id)
    ```
    </CodeWrap>
...
```
  ...
  <Tab title="Java">
    <CodeWrap title="CustomerRegistration.java" fileUrl="https://github.com/versa-protocol/java-sender/blob/main/src/main/java/org/versa/sender/CustomerRegistration.java">
    ...
    CustomerRegistrationRequest registrationPayload = new CustomerRegistrationRequest(
      ...
      RestTemplate restTemplate = new RestTemplate();
      ResponseEntity<String> response = restTemplate.exchange(
        "https://registry.versa.org/handle",
        HttpMethod.POST,
        entity,
        String.class
      );
  ...
  <Tab title="NodeJS">
    <CodeWrap title="customer_registration.ts" fileUrl="https://github.com/versa-protocol/nodejs-client/blob/main/src/lib/customer_registration.ts">
