...
Optionally, if a sender knows the receiver's client ID, they may register their customers to forward receipts to that receiver.
...
Receipts are routed by customer "handles" — such as email address and domain.
To send receipts for a customer to a specific receiver, use the handle registration API and provide the receiver's client ID.
Future receipts matching that customer will be forwarded appropriately.
[Learn more](/customer_registration)
