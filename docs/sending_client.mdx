...
Developer documentation for implementing a self-hosted Versa sender client, using the [Versa REST API](/sending). We currently maintain example client implementations for Rust and [Java](https://github.com/versa-protocol/java-sender). Contact us if you'd like assistance with another language.

> [!TIP]
> Looking for a quicker start? You can use our [custodial configuration](/sending#custodial-configuration) for faster onboarding.
...
The 32-byte key is provided by the registry. Generate as 12-byte nonce for each receiver and send the nonce with the encrypted data to each receiver.
...
<TabGroup>
  <Tab title="Rust">
    ...
    <CodeWrap title="lib.rs" fileUrl="https://github.com/versa-protocol/rust-client/blob/main/protocol/src/lib.rs">
