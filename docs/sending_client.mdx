...
Developer documentation for implementing a self-hosted Versa sender client, using the [Versa REST API](/sending). We currently maintain example client implementations for [Rust](https://github.com/versa-protocol/rust-client) and [Java](https://github.com/versa-protocol/java-sender). Contact us if you'd like assistance with another language.
...
Congratulations! You've successfully sent a secure receipt to an authorized receiver on the Versa network.
They will complete the process by checking out the key from the registry and decrypting the data on behalf of your customer.
