...
Versa Link is one method of customer registration, in which most of the work is handled via the embeddable widget. Alternatively, you may use the developer portal to whitelist customers, or implement your own via the [API](/customer_registration#api-access).
...
1. [Generate a link token](#generate-a-link-token) and pass the temporary token to your app’s client.
2. Use the link token to [initialize the Versa Link widget](#initialize-versa-link).
3. [Launch the widget](#launch-the-widget) by calling the `open` method.
...
curl -X POST https://registry.versa.org/link_token \
    ...
    -d '{
      "references": [{
        "handle_type": "merchant_group_code",
        "handle": "CORPORATE_DISCOUNT_CODE_XYZ"
      },
      {
...
<CodeWrap title="onSuccess Example">
```js
...
| ------------- | ------- | ----------------------------------------------------------------------------------------- |
| `changes`     | `array` | What changed in this Link session. See [change object](#change-object) definition.        |
| `connections` | `array` | All currently active connections. See [connection object](#connection-object) definition. |
...
## Advanced Use
The following configuration options are not necessary for a basic Link integration, but may be useful in certain cases.

### Read-Only Access Mode
To use Link for viewing active connections without allowing users to make changes, you may create the link token with an optional `access_mode` parameter set to `read_only`. This will cause the Link widget to render in a read-only mode, hiding any buttons or other interactivity for making changes to connections. The `access_mode` parameter accepts either `read_only` or `read_write` as values, but defaults to `read_write` if not specified.
