...
When a user successfully completes a connection via the Link flow, the `onSuccess` callback receives a response object. This object contains information about the changes that occurred during the Link session and the current state of all connections.
