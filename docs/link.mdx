...
When a user completes the Link flow, your onSuccess callback receives a response object:
...
- changes → What changed in this Link session.
- connections → All currently active connections.
...
The `onSuccess` callback is invoked when a user successfully completes a connection flow in the Versa Link widget. It receives a single parameter containing information about what changed and the current state of all connections.
...
|----------|------|-------------|
| `changes` | `ConnectionChange[]` | Array of changes made during this session |
| `connections` | `Connection[]` | Array of all currently active connections |
...
| Property | Type | Description |
|----------|------|-------------|
| `org` | `Org` | Details about the receiver organization |
| `oauth` | `boolean` | Whether this connection uses OAuth for setting up the connection |
| `category` | `ReceiverCategory` | The category of the receiver: `'expense'`, `'firehose'`, or `'duty_of_care'` |
| `action` | `'enabled' \| 'disabled'` | The action performed on this connection |
...
| `oauth` | `boolean` | Whether this connection uses OAuth for setting up the connection |
| `category` | `ReceiverCategory` | The category of the receiver: `'expense'`, `'firehose'`, or `'duty_of_care'` |
| `handling` | `Handling` | Registration status information |
