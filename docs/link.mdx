...
Before you initialize Versa Link, use your server-side credentials to acquire a short-lived token to pass to the client.
...
curl -X POST https://registry.versa.org/link_token \
    ...
    -d '[
      ...
      },
      {
        ...
        "associated_handles": [
          { handle: "acme.co", handle_type: "customer_email_domain" },
          { handle: "CORPORATE_DISCOUNT_CODE_XYZ", handle_type: "merchant_group_code" }
        ]
      },
      {
        "model": "receiver",
...
await VersaLink.initialize({
    ...
    onSuccess: (result) => {
      console.log('Versa Link success:', result.integrations.map(i => i.name).join(', '));
    },
    onExit: () => {
