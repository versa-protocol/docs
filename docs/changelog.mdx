...
If you are a sender, make a plan to migrate your data mapping from `1.11.0`.
If you are a receiver, update your webhooks to handle the new version alongside LTS versions (`1.11.0`),
based on the incoming data's schema version.

If you are a custodial client, you may have to manually update your webhook's maximum schema version to start receiving data of the newer version.
Until you do, data will be 'downshifted' to fit the `1.11.0` schema.
...
metadata | array of metadatum | No | List of metadata items

### End of Life

Schema Version `1.10.0` and earlier are now considered at end of life (EOL) and are no longer supported.
The registry will not accept new registrations for these versions.
...
- Added a `ReceiptWithHistory` component to the [@versaprotocol/react library](https://www.npmjs.com/package/@versaprotocol/react) that supports viewing previous versions of a receipt
