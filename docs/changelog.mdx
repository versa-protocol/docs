...
## 2026-04-20 - Schema Version 2.3.0

- Added nullable `trip` object (`id`, `name`, `description`) to the `header`, for grouping related transactions under a broader trip
- Added nullable `lifecycle_status` enum to the `header`, with values `active`, `canceled`, or `refunded`
- Added nullable `booked_at` timestamp to the `header`
- Added dedicated [`booking`](/booking) and `invoice` event schemas
- Made `odometer_reading_in` and `odometer_reading_out` optional on the `car_rental` object

## 2026-04-18 - Protocol Update

- Migrated event ID generation in the registry from `uuidv4` to `uuidv7` to improve sortability and traceability, conforming to the [typeid spec](https://github.com/jetify-com/typeid/tree/main/spec)

Already issued IDs will continue to be valid and accepted by the registry, but all new objects will be issued v7-based typeids.

## 2026-03-18 - Schema Version 2.2.0

- Added nullable `confirmation_number` and `record_locator` to both the `lodging` and `car_rental` objects
- Added nullable `chain_code` and `property_id` to the `lodging` object
- Added nullable `vendor_code` to the `car_rental` object
- Added nullable `booker` ([Person](https://docs.versa.org/schema#person) object) to the `customer` object

## 2025-12-01 - Schema Version 2.1.1

This is a patch update to correct small inconsistencies in the 2.x channel. For most clients, no changes will be required to support this schema version.

- Fixes non-optional tax `rate` (all nullable fields should be optional and vice versa)
- Accept any ISO 4217 lower-case `currency` code using a regex pattern instead of an enum
- Apply the `person` object migration to alpha `car_rental` schema
  - Removed `driver_name` from `car_rental` itemization
  - Added nullable `drivers` array of `person` objects to `car_rental` itemization
...
- `passenger` changes from a string to a new nullable `person` object (see the [Person documentation](/schema#person) for details)
