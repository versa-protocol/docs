...
The Versa Protocol is designed for data transfer without the need to trust a third-party. Senders (merchants) send receipts directly to customers’ financial services (their bank, their expense management software, etc.).

The receipt data never touches Versa servers. Instead, the Versa Registry acts like DNS on the web: the registry returns a list of authorized receivers for any given sale, based on details about the customer (either the domain of the customer’s email, or a card reference). The rest of the exchange is handled directly between senders and registered receivers.
