...
The Versa protocol helps merchants securely send business receipt data to customers.

Gathering receipts can be a hassle (each employee snapping photos of paper receipts, taking screenshots of email receipts, etc.). With Versa, this process is automated: merchants use a unified API to send digital receipts directly to customers’ authorized financial apps (their bank or spend management software).
...
For any given sales receipt, the Versa Registry returns a list of matching authorized receivers. This list of authorized receivers is determined on the basis of a few key transaction details (the domain of the customer’s work email, a bank routing number, or a card identifier).
