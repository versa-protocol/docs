...
You can cancel a transaction while it is still in an active lifecycle state. This may prevent a charge for receipt retrieval. Identify the event by its `id` or by the `merchant_transaction_id` you provided at creation.
