...
For SaaS bills, phone bills, etc. At least one subscription item within the subscription array should be type `recurring`.
...
| Field          | Type    | Description                                     |
| -------------- | ------- | ----------------------------------------------- |
| `subscription` | `array` | An array of [subscription item](#item) objects. |
...
| `interval_count`       | `inullable nteger` | The number of intervals (specified in the interval attribute) between subscription billings. For example, interval=month and interval_count=3 bills every 3 months. |
...
| `metadata`             | `nullable array`   | Array of key/value [metadata pairs](#item-metadata), e.g. SKU.                                                                                                      |
...
| `type` | `nullable enum` | One of `rebuy`. May be used to apply an icon to the action button. |
...
<details>
<summary>Item Metadata Primitive</summary>
<article>
...
> [!TIP]
> Either use the `type` field or the `name` field, not both.

| Field   | Type              | Description                                                                                   |
| ------- | ----------------- | --------------------------------------------------------------------------------------------- |
| `type`  | `nullable enum`   | One of `sku`, `unspsc`, `asin`. Otherwise leave null and use `custom_key`.                    |
| `name`  | `nullable string` | The type of metadata, e.g. 'Size', 'Brand', 'Product Condition', 'Special Instructions', etc. |
...
| `type`   | `nullable enum`  | One of `fixed` or `percentage`.                                        |
