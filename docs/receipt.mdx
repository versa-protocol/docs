...
The rest of the receipt is composed of the four objects described above: `header`, `itemization`, `actions`, and `payment`. Below we cover each of these four sections in detail.
...
| `discounts`   | `nullable array`  | Array of [discounts](#discount) applied at the top level. These are independent of line item discounts, which are added at the line item level. |
