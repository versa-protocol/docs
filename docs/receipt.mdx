...
4. [**Payments**](#payments): Payment status of the receipt.
...
| `paid`        | `integer`         | Sum of all payments. Will equal the total if the invoice is paid in full.                                           |
...
| `invoice_level_adjustments` | `nullable array` | Array of [adjustments](#adjustment) applied at the top (invoice) level. These are independent of line item adjustments, which are added at the line item level. |
...
| `vehicle`                   | `string`           | A [vehicle](#vehicle) object.                                                                                                                                   |
...
| `departure_timezone`     | `nullable string`  | A list of possible time zone values is maintained at the [IANA Time Zone Database](https://www.iana.org/time-zones). |
| `arrival_timezone`       | `nullable string`  | A list of possible time zone values is maintained at the [IANA Time Zone Database](https://www.iana.org/time-zones). |
| `flight_number`          | `nullable string`  |                                                                                                                      |
| `class_of_service`       | `nullable string`  |                                                                                                                      |
| `taxes`                  | `nullable object`  |                                                                                                                      |
| `adjustments`            | `nullable array`   | Array of [adjustment](#adjustment) objects.                                                                          |
...
| Field                       | Type             | Description                                                                                                                                                     |
| --------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lodging_items`             | `array`          | An array of [lodging item](#lodging-item) objects.                                                                                                              |
| `invoice_level_adjustments` | `nullable array` | Array of [adjustments](#adjustment) applied at the top (invoice) level. These are independent of line item adjustments, which are added at the line item level. |
...
| `departure_at`       | `integer`         | Departure time. Measured in seconds since the Unix epoch.                                                         |
| `arrival_at`         | `integer`         | Arrival time. Measured in seconds since the Unix epoch.                                                           |
...
| `total`                | `integer`          | Total for the line item (typically `quantity` × `unit_cost` - `adjustments` + `taxes`).                                                                             |
...
| Field                       | Type             | Description                                                                                                                                                     |
| --------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `invoice_level_line_items`  | `nullable array` | An array of [item](#item) objects at the invoice level. For items that are part of shipment, move them under the [shipment](#shipment) object.                  |
| `invoice_level_adjustments` | `nullable array` | Array of [adjustments](#adjustment) applied at the top (invoice) level. These are independent of line item adjustments, which are added at the line item level. |
| `shipments`                 | `nullable array` | An array of [shipment](#shipment) objects.                                                                                                                      |
...
> [!NOTE]
> For security reasons, we recommend that the action urls share the same domain as the sending merchant, wherever possible. Action URLs that don’t share a domain with the sending merchant may be filtered out by the receiver.
...
| Field             | Type              | Description                                                                                                                   |
| ----------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `amount`          | `int`             | The total of the adjustment, in the [smallest currency unit](#currency-values). Positive for costs, negative for adjustments. |
| `adjustment_type` | `enum`            | One of `discount`, `tip`, `fee`, `other`.                                                                                     |
| `name`            | `nullable string` | The name of the adjustment.                                                                                                   |
| `rate`            | `nullable float`  | The adjustment rate, e.g. 0.1 for 10%. Null for fixed adjustment.                                                             |

</article>
</details>

<details>
<summary>Doc Primitive</summary>
<article>

### Doc

A text blob, to be used for terms and conditions, agreements, or other unstructured information.

| Field   | Type     | Description                                                |
| ------- | -------- | ---------------------------------------------------------- |
| `title` | `string` | The title of the text document, e.g. 'Terms & Conditions'. |
| `body`  | `string` | The doc body.                                              |
