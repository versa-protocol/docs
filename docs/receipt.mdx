...
2. [**Itemization**](#itemization): Description, quantity, unit cost, and total for each line item. Type-specific templates describe special types of line items, e.g. flight, e-commerce purchase, hotel stay.
...
The rest of the receipt is composed of the five objects described above: `header`, `line_items`, `roll_up`, `actions`, and `payment`. Below we cover each of these five sections in detail.
...
| `location`    | `nullable object`  | Brick-and-mortar [place](#place) object for in-person purchases..                                                   |
...
Used in cases where the merchant has a profile for the customer. Can be left null in guest checkout flows. The email domain is sometimes used to map the receipt to the buyer.
...
The Itemization section describes the goods and services that make up the transaction. This section must conform to one of seven templates, according to the category of goods and services being described: General, Car Rental, E-commerce, Flight, Lodging, Subscription, or Transit Route. Exactly one line item grouping must be non-null.
