...
2. [**Line Items**](#line-items): Description, quantity, unit cost, and total for each line item. Type-specific objects describe special types of line items, e.g. flight, ecommerce purchase, hotel stay.
...
Line items describe the goods and services that make up the transaction. Line items fall into one of seven categories: General, Car Rental, E-commerce, Flight, Lodging, Subscription, or Transit Route. Exactly one line item grouping must be non-null.
