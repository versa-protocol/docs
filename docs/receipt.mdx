...
A Versa receipt is wrapped in an object that includes the sender reference and schema version.
...
The two wrapper fields — `sender_id` and `schema_version` — are used by the receiver to decrypt and validate the receipt.

| Field            | Type      | Description                                                                                                                     |
| ---------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `sender_id`      | `string`  | Unique merchant identifier for the sender. The receiver will expand this id into a full [merchant](merchant-primitive) profile. |
...
The Type Block section consists of a series of templates specific to different receipt types. Versa covers the following types: `car_rental`, `flight`, `location`, `lodging`, `transit_route`, `subscription`, `shipment`.
