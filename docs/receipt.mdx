...
The two wrapper fields `sender_id` and `schema_version` are used in the decryption and validation process.
...
Use the optional third party object when the sender is a platform. For example, if a coffee shop uses Square to process sales, the ‘sending merchant’ would be Square and the ‘third party merchant’ would be the coffee shop. The `make_primary` flag specifies whether the sending merchant or the third party merchant should get top billing on the receipt.
