...
1. [Header](#header): Top-level fields, e.g. merchant name, merchant logo, purchase date.
2. [Type Block](#type-block): Type-specific objects describing the purchased goods and service. E.g. check-in and check-out information for a hotel stay, origin and destination information for a ticketed flight. Note: this section is only present for special receipt types, and is otherwise absent.
3. [Line Items](#line-items): Description, quantity, unit cost, and total for each line item.
4. [Roll-up](#roll-up): Subtotal, taxes, fees, tip, discounts, and grand total of the receipt.
...
6. [Payment](#payemnt): Payment status and metadata associated with the receipt.
...
| `amount`      | integer          | Receipt total amount. This amount is in the receipt currency and in the [smallest currency unit](#currencies). |
...
| `logo`        | nullable strng  | Logo image url, in a square aspect ratio. Recommmend size 400x400. |
...
| `icon`        | nullable string | For certain action types, a reinforcing icon. Currently, the only supported icon is `rebuy`. |
