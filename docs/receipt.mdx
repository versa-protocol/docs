...
| `acriss_car_code`      | `nullable string`  | Four-letter Association of Car Rental Industry Systems Standard (ACRISS) car code. |
...
```JSON
"subscription": {
  "subscription_renew_date": 1716405417
}
```

| Field                     | Type      | Description                                             |
| ------------------------- | --------- | ------------------------------------------------------- |
| `subscription_renew_date` | `integer` | Renewal date. Measured in seconds since the Unix epoch. |
