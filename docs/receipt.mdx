...
After these two wrapper fields, `sender_id` and `schema_version`, the receipt is composed of the six objects outlined above. Below we cover each of these six sections in detail.
...
The merchant object is applied both as a child of the `third_party` object, and as the expanded `sender_id` via the Versa registry.
